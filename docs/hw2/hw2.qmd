...
\begin{equation}\tag{5}\label{eq:vibstring}
...
\begin{align}
	\rho_0 \frac{d^2u}{dt^2} -T_0 \frac{d^2u}{dx^2} +\beta \frac{du}{dt} = 0 \\
	\frac{\rho_0 u_s}{t_s^2}\frac{d^2\hat{u}}{d\hat{t}^2} - \frac{T_0 u_s}{x_s^2}\frac{d^2\hat{u}}{d\hat{x}^2} + \beta\frac{u_s}{t_s}\big(\frac{d\hat{u}}{d\hat{t}}\big) = 0 \\
	\frac{d^2\hat{u}}{d\hat{t}^2} - \frac{T_0 t_s^2}{\rho_0 x_s^2}\frac{d^2\hat{u}}{d\hat{x}^2} + \beta\frac{t_s}{\rho_0}\big(\frac{d\hat{u}}{d\hat{t}}\big) = 0
\end{align}

Let

\begin{align}
	\frac{\beta t_s}{\rho_0} = 1 \\
	t_s = \tau = \frac{\rho_0}{\beta}
\end{align}

where $\tau$ is the characteristic time scale for (\ref{eq:vibstring}).

\begin{align}
	\frac{T_0}{\rho_0 x_s^2} = 1 \\
	x_s = L_c = \sqrt{\frac{T_0}{\rho_0}}
\end{align}

where $L_c$ is the characteristic length scale for (\ref{eq:vibstring}).

Let us solve the non-dimensionalized IBVP

\begin{equation}\tag{6}\label{eq:nondimvibstring}
	\begin{cases}
		u_{tt} + u_t = \tau^2 u_{xx} \\
		u(0, t) = 0 & u(L, t) = 0 \\
...
\begin{align}
	XT'' + XT' = \tau^2 X''T \\
	\frac{T'' + T'}{\tau^2 T} = \frac{X''}{X} = \lambda
\end{align}
...
$\begin{align}
	\begin{cases}
		...
		X(0) = X(L) = 0
	\end{cases}
\end{align} \Longrightarrow X_n(x) = \sin{(\frac{n \pi x}{L})} \quad n = 1, 2, 3, ...$
...
\begin{align}
	r^2 + r - \tau^2 \lambda_n = 0 \\
	r = -\frac{1}{2} \pm \frac{1}{2}\sqrt{1 + 4\tau^2 \lambda_n}
\end{align}

we know that $\tau^2 \lambda_n < 0$
...
\begin{equation}\tag{7}\label{eq-laplace}
	\begin{cases}
		u_{xx} + u_{yy} = 0 \\
		u_x(0, y) = 0 & u_y(x, 0) = 0 \\
